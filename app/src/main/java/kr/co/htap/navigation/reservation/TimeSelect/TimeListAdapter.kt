...
class TimeListAdapter(
    private val timeList: ArrayList<TimeDTO>,
    private val documentId: String,
    private val date: DateDTO
) : RecyclerView.Adapter<TimeListAdapter.MyViewHolder>() {
    inner class MyViewHolder(binding: ReservationTimeButtonBinding) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        val binding =
            ReservationTimeButtonBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return MyViewHolder(binding)
    }

    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        val minute: String =
            if (timeList[position].minute < 10) "0" + timeList[position].minute else timeList[position].minute.toString()
        holder.button.text = timeList[position].hour.toString() + ":" + minute
        holder.button.paintFlags =
            if (timeList[position].isAvailable) 0 else Paint.STRIKE_THRU_TEXT_FLAG
